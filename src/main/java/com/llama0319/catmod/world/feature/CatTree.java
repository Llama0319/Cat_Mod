...
import com.llama0319.catmod.client.util.DirectionUtil;
...
public class CatTreeFeature extends Feature<NoFeatureConfig> {
	private static final BlockState TRUNK = BlockInit.CAT_LOG.get().getDefaultState();
	private static final BlockState LEAF = BlockInit.CAT_LEAVES.get().getDefaultState().with(LeavesBlock.DISTANCE, 1);
	
	
	
	
	public CatTreeFeature(Codec<NoFeatureConfig> p_i231953_1_) {
		super(p_i231953_1_);
	}
	...
	private void branchSpawn(ISeedReader worldIn, Random rand, BlockPos middle) {
		Direction randomDirection = DirectionUtil.DIRECTIONS_4h[rand.nextInt(4)];
		if (rand.nextBoolean()) {
			ifAirSetBlock(worldIn, middle.offset(randomDirection, 1),
					TRUNK.with(RotatedPillarBlock.AXIS, randomDirection.getAxis()));
			ifAirSetBlock(worldIn, middle.offset(randomDirection, 2), LEAF);
		} else {
			ifAirSetBlock(worldIn, middle.offset(randomDirection, 1),
					TRUNK.with(RotatedPillarBlock.AXIS, randomDirection.getAxis()));
			ifAirSetBlock(worldIn, middle.offset(randomDirection, 2),
					TRUNK.with(RotatedPillarBlock.AXIS, randomDirection.getAxis()));
	...
	private void longBranchSpawn(ISeedReader worldIn, Random rand, BlockPos startPosition, Direction randomDirection) {
		...
		for (int i = 1; i <= 2 + rand.nextInt(3); i++) {
			...
			if (rand.nextInt(3) == 0)
				startPosition = startPosition.offset(addedDirection);
	...
	@Override
	public boolean generate(ISeedReader worldIn, ChunkGenerator chunkGenerator, Random rand, BlockPos positionIn,
			NoFeatureConfig config) {
		// Moving down until it is on the ground
		while (positionIn.getY() > 1 && isAirOrLeavesAt(worldIn, positionIn))
			positionIn = positionIn.down();

		if (!isDirtAt(worldIn, positionIn)
				&& !(worldIn.hasBlockState(positionIn, state -> state.getBlock().equals(BlockInit.PINK_GRASS.get())))
				&& !(worldIn.hasBlockState(positionIn, state -> state.getBlock().equals(BlockInit.PINK_GRASS.get())))) {
			return false; // this tree is only allowed to grow on soil, but not on water or plant or other
							// thing
		}
		// Make main trunk
		...
		// Top leaves
		ifAirSetBlock(worldIn, new BlockPos(continuePos.getX(), continuePos.getY() + hight2 + 1, continuePos.getZ()),
				LEAF);
		if (rand.nextBoolean()) {
			ifAirSetBlock(worldIn,
					new BlockPos(continuePos.getX(), continuePos.getY() + hight2 + 2, continuePos.getZ()), LEAF);
		}
